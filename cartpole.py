...
def choose_action(state, mdp_data):
    ...
    # *** START CODE HERE ***
    # print(mdp_data)
    tran_probs = mdp_data['transition_probs']
    value = mdp_data['value']
    # print(tran_probs.shape)
    # print(value.shape)

    exp = tran_probs[state].T @ value
    ...
    if exp[0] > exp[1]:
        ...
        # choosing the random action like this, changes the outputs. But overall, we get similar plots for the two action with different seeds (see pdf)
...
def update_mdp_transition_counts_reward_counts(mdp_data, state, action, new_state, reward):
    ...
    # *** START CODE HERE ***
    # print(mdp_data['transition_counts'])
    # print(mdp_data['reward_counts'])
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    # *** START CODE HERE ***
    nums = mdp_data['transition_probs'].shape
    temp_probs = mdp_data['transition_probs']
    # probs = mdp_data['transition_probs']
    tcounts = mdp_data['transition_counts']
    for k in range(nums[0]):
        total_action = np.sum(tcounts[k], axis = 0)
        temp_probs[k] = tcounts[k] / total_action

    mdp_data['transition_probs'] = np.nan_to_num(temp_probs, copy = False, nan = 1/mdp_data['num_states'])
    

    # nums = mdp_data['rewards'].shape
    # for j in range(nums[0])
     # avg reward observed in state s
    rcounts = mdp_data['reward_counts']
    temp = -rcounts[:,0] / rcounts[:,1]
    mdp_data['reward'] = np.nan_to_num(temp, copy=False, nan=0)
    # *** END CODE HERE ***
...
def update_mdp_value(mdp_data, tolerance, gamma):
    ...
    while True:
        #value iteration
        # print(mdp_data['value'] @ probs)
        new = reward + gamma * np.max(value @ probs, axis= 1)

        counter +=1
        print('iter', counter)

        if (np.max(abs(new - value)) < tolerance):
            mdp_data['value'] = new
            if counter == 1:
                return True
            else:
...
def main(plot=True):
    ...
    if plot:
        ...
        plt.savefig('./control' + str(seed)+'.pdf')
